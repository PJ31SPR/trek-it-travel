...
const Profile = () => {
  ...
  const pickImage = async () => {
    ...
    const addUserData = async (userId, newData) => {
      try {
        // await db.collection("users").doc(userId).update({
          //   avatar: result.assets[0].uri
          const userDocRef = doc(db, "users", userId);
          // Add the new data to the user document
    ...
    if (!result.canceled) {
      ...
      const newData = { photoURL: result.assets[0].uri }
      // Save the avatar URL to Firestore
      addUserData(userId, newData)
      };
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.contentContainer}>
        ...
        <View style={styles.buttonsContainer}>
          <Pressable
            style={[styles.button, styles.buttonPressable]}
            onPress={() => {}}
          >
